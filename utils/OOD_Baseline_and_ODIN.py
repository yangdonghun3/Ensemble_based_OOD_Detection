...
def Baseline_and_ODIN(pre_trained_net, in_dataset, out_dataset):
    ...
    # load networks
    if pre_trained_net == "resnet_cifar100":
        num_classes = 100
    else :
        num_classes = 10
    try:
        model = models.resnet.ResNet34(num_c=num_classes)
    ...
    for T in T_list:
        for m in M_list:
            magnitude = m
            temperature = T
            lib_generation_5.get_posterior(model, test_loader, magnitude, temperature, outf, True, in_dataset_name)
            out_count = 0
            print('Temperature: ' + str(temperature) + ' / noise: ' + str(magnitude)) 
            lib_generation_5.get_posterior(model, out_test_loader, magnitude, temperature, outf, False, in_dataset_name)
