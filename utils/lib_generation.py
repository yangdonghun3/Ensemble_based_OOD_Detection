...
def sample_estimator(model,base_model, num_classes, feature_list, train_loader):
    ...
    for data, target in train_loader:
        ...
        if len(base_model.feature_list(data)) == 2:
            out_features_b = base_model.feature_list(data)[1]
        else :
            out_features_b = base_model.feature_list(data)
        for i in range(len(out_features_b)):
            out_features.append(out_features_b[i])
        # get hidden features
        for i in range(num_output):
...
def get_Mahalanobis_score(model, base_model, test_loader, num_classes, outf, out_flag, sample_mean, precision, layer_index, magnitude,in_dataset_name):
    ...
    for data, target in test_loader:
        ...
        if (layer_index >=0 and layer_index <=4):
            out_features = model.intermediate_forward(data, layer_index)
            out_features = out_features.view(out_features.size(0), out_features.size(1), -1)
            out_features = torch.mean(out_features, 2)
        elif (layer_index >4 and layer_index <=10):
            out_features = base_model.intermediate_forward(data, layer_index-5)
            out_features = out_features.view(out_features.size(0), out_features.size(1), -1)
            out_features = torch.mean(out_features, 2)            
        ...
        if (layer_index >=0 and layer_index <=4):         
            ...
            noise_out_features = base_model.intermediate_forward(Variable(tempInputs, volatile=True), layer_index-5)
            noise_out_features = noise_out_features.view(noise_out_features.size(0), noise_out_features.size(1), -1)
            noise_out_features = torch.mean(noise_out_features, 2)        
