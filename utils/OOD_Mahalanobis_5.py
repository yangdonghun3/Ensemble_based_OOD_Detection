...
import models
import os
from utils import lib_generation_5
...
def Mahalanobis_Generate_5(pre_trained_net, train_dataset, in_dataset, out_dataset):
    ...
    #model.load_state_dict(torch.load(model_path, map_location = "cuda:" + str(0)))
    #print(model_path)
    ...
    sample_mean, precision = lib_generation_5.sample_estimator(model, num_classes, feature_list, train_loader)
    ...
    for magnitude in m_list:
        ...
        for i in range(num_output):
            M_in = lib_generation_5.get_Mahalanobis_score(model, test_loader, num_classes, outf, \
        ...
        for i in range(num_output):
            M_out = lib_generation_5.get_Mahalanobis_score(model, out_test_loader,num_classes, outf, \
        ...
        Mahalanobis_data, Mahalanobis_labels = lib_generation_5.merge_and_generate_labels(Mahalanobis_out, Mahalanobis_in)
        file_name = os.path.join(outf, 'Mahalanobis_%s_%s_%s.npy' % (str(magnitude), in_dataset_name , out_dataset_name))
        Mahalanobis_data = np.concatenate((Mahalanobis_data, Mahalanobis_labels), axis=1)
        np.save(file_name, Mahalanobis_data)
        
def Mahalanobis_Regression_5(pre_trained_net, in_dataset, out_dataset):
    temp=in_dataset.root    
    in_dataset_name = temp.split('/')[-1]
    in_dataset_name = in_dataset_name.split('\\')[-1]
