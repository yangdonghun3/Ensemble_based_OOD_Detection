...
def Mahalanobis_Generate(pre_trained_net, base_model, train_dataset, in_dataset, out_dataset):
    base_model_path = './trained_models/'+base_model+'.pth'
    ...
    try:
        base_model = base_model.embedding_net
    ...
    in_dataset_name = in_dataset_name.split('\\')[-1]

    temp=out_dataset.root    
    out_dataset_name = temp.split('/')[-1]
    out_dataset_name = out_dataset_name.split('\\')[-1]

    

    if in_dataset_name== 'MNIST' or in_dataset_name=="FashionMNIST":
        temp_x = torch.rand(128,1,28,28).cuda()
    else:
        temp_x = torch.rand(128,3,32,32).cuda()
    ...
    if len(base_model.feature_list(temp_x)) == 2:
        temp_list_b = base_model.feature_list(temp_x)[1]
    else :
        temp_list_b = base_model.feature_list(temp_x)
    for i in range(len(temp_list_b)):
        temp_list.append(temp_list_b[i])
    ...
    sample_mean, precision = lib_generation.sample_estimator(model, base_model, num_classes, feature_list, train_loader) 
    ...
    for magnitude in m_list:
        ...
        for i in range(num_output):
            ...
            M_in = lib_generation.get_Mahalanobis_score(model,base_model, test_loader, num_classes, outf, \
        ...
        for i in range(num_output):
            M_out = lib_generation.get_Mahalanobis_score(model,base_model, out_test_loader,num_classes, outf, \
